...
export class MagazineBranchesService {
  ...
  create(idCode:string, name:string, lct: string, storeID: number, token: string) {
    let headerSettings = {};
    headerSettings['Authorization'] = `Bearer ${token}`;
    let headers = new HttpHeaders(headerSettings);
    return this.http.post(`${environment.connectionURL}/MagazineBranches`, { identificationCode: idCode, name: name, location: lct, magazineId: storeID }, {headers: headers});
  }
  ...
  edit(idCode: string, name: string, id: number, lct: string, token: string) {
    ...
    headerSettings['Authorization'] = `Bearer ${token}`;
    let headers = new HttpHeaders(headerSettings);
    return this.http.put(`${environment.connectionURL}/MagazineBranches`, { identificationCode: idCode, location: lct, name: name, id: id, magazineId: environment.id}, {headers: headers});
  }
